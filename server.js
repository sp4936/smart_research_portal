    ...
    exec(`python leadAssignment.py "${INDUSTRY_TYPE}" ${assets}`, (error, stdout, stderr) => {
      ...
      try {
        ...
        res.status(200).json({
          ...
          team: output.team,
          industryType: output.industry,
          totalAssets: output.assets,
...
app.post('/api/search', async (req, res) => {
  ...
  switch (dataType) {
    case 'COMBINED_DATA':
      query = `SELECT SOURCE as Source,	CUSTOMER_NAME as "Customer Name", ADDRESS,	WEBSITE,	ZIP_CODE as "Zip Code",	RSSD_ID as "RSSD ID", CERTIFICATE_NUMBER as "Certificate Number",INDUSTRY_TYPE as "Industry Type",	TOTAL_ASSETS as "Total Assests"
      ,	CIK_ID as "CIK ID",	FDIC_ID as "FDIC ID",	FAX,	PHONE,	CITY,	STATE,INACTIVE,	NET_INCOME,	SOURCE_WEBSITE,	ABA_NUMBER,	LAST_UPDATED_DATE FROM dev_fccdw.CS_CUSTOMER_RESEARCH_DATA`;
      break;
    case 'SNP_GLOBAL_DATA':
      query = `SELECT ENTITY_NAME as "Entity Name", ENTITY_ID as "Entity ID", INDUSTRY_CLASSIFICATION as "Industry Classification", INDUSTRY_CLASSIFICATION_2 as "Industry Classification 2", TOTAL_ASSETS as "Total Assests", WEBSITE, ADDRESS1, ADDRESS2, CITY, STATE, COUNTRY_NAME as "Country Name", SIC_CODE, PARENT_NAME, PARENT_CITY, WEB_URL, SOURCE, ETL_LOAD_DATE  FROM dev_fccdw.STG_SNP_GLOBAL_DATA`;
      break;
    case 'DNB_DATA':
      dbConfigToUse = dbConfig2;
      //query = `SELECT * FROM fccdw.DUN_AND_BRADSTREET`;
      query = `SELECT 
      TO_NUMBER(DUNS_NUMBER) AS "DUNS Number", 
      BUSINESS_NAME AS "Business Name", 
      TRADESTYLE_NAME AS "Tradestyle Name", 
      STREET_ADDRESS AS "Street Address", 
      STREET_ADDRESS_2 AS "Street Address 2", 
      CITY_NAME AS "City", 
      STATE_PROVINCE_ABBREVIATION AS "State Abbreviation", 
      STATE_PROVINCE_NAME AS "State Name", 
      TO_NUMBER(POSTAL_CD) AS "Postal Code", 
      COUNTRY_NAME AS "Country", 
      COUNTY_NAME AS "County", 
      TO_NUMBER(SALES_VOLUME_US_DOLLARS) AS "Sales Volume (USD)", 
      TO_NUMBER(EMPLOYEES_TOTAL) AS "Total Employees", 
      TO_NUMBER(GLOBAL_ULT_DUNS_NUMBER) AS "Global Ultimate DUNS Number", 
      GLOBAL_ULT_BUSINESS_NAME AS "Global Ultimate Business Name", 
      LAST_UPDATE_DATE AS "Last Update Date", 
      LINE_OF_BUSINESS AS "Line of Business", 
      US_1987_SIC_1 AS "US 1987 SIC 1" 
    FROM fccdw.dun_and_bradstreet
    `;
      break;
    default:
